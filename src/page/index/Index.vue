<template>
  <div class="index">
    ...
    <div class="my-swiper">
      <mt-swipe :auto="4000">
        <mt-swipe-item v-for='list in swiper_list'><img :src="'http://www.hzkygd.cn/traditionalChineseMedicine'+list.bannerImage"></mt-swipe-item>
    ...
    <div class="hot">
      ...
      <div class="doctor-box">
        <router-link  v-for='doctor in hot_doctor_list' :to="{path:'/detail',query: {id:doctor.id}}">
          <img :src="'http://www.hzkygd.cn/traditionalChineseMedicine'+doctor.headImage">
      ...
      <div class="server-box" style="margin-top:0.266667rem">
        <div style="background:#fff">
          <router-link v-for='list in property_list' :to='{path:"/doctor",query:{id:list.id}}'>
            ...
            <b>
                <img :src="'http://www.hzkygd.cn/traditionalChineseMedicine'+list.imagePath">
    ...
    <div class="refer-center">
      <h5 style="margin-bottom:0.266667rem"><span>资讯中心 </span><i class="icon"><router-link to='/consult'>更多&#xe606;</router-link></i></h5>
      <ul class="refer-box" style="margin-top:0.266667rem">
        ...
        ='margin-top:0.266667rem'>
          ...
          <div class="consult-list">
            ...
            <div v-html="list.message" class="consultMessage">
              {{list.message}}
            </div>
          </div>
        </router-link>
...
export default {
  ...
  mounted () {
    ...
    var vm =this
    var token = ''
    // var URIstring ='http://www.hzkygd.cn/web_medicine/h5/index.html#/mobiledetail?id=30'
    // encodeURIComponent()
   // if(localstorage.getItem('medicine')&&vm.$store.state.data.im_start=='1'){
   //    this.init_im()
   //    this.storeImStart(0)
   // }else{

   // }
   // if(_url.indexOf('code=')&&vm.$store.state.data.im_start=='1'){
   //  this.code =_url.substring(_url.indexOf("code=")+5,_url.indexOf("&"))
   //  this.login_ajax(this.code)
   // }
   this.login_ajax()
  },
  created () {
  ...
  methods : {
    ...
    ...mapMutations([
        'storeImStart','storeMessageHas','storeShouquan'
      ]),
    login_ajax:function(){
      var vm = this 
      var _url = window.location.href
      vm.code = _url.substring(_url.indexOf("code=")+5,_url.indexOf("&"))
      console.log(vm.code,'-||||||||||||')
      vm.$http.post('/traditionalChineseMedicine/userinfo/weiXinLogin',{
        code:vm.code
      },{emulateJSON:true}).then(function(res){
        console.log(res,'微信说我群')
        if(res.body.errcode){
          if(vm.$store.state.data.im_start=='1'){
            this.init_im()
            this.storeImStart(0)
          }
          ...
          new Promise(function(resolve,reject){
            ...
            console.log(window.localStorage.getItem('medicine'),"|||||||||||||||||||||||+++++++++")
            vm.init_im()
            vm.storeImStart(0)
          })
