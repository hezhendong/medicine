...
export default {
  ...
  mounted () {
    ...
    this.my_user_id = JSON.parse(localstorage.getItem('medicine')).id
  ...
  methods : {
        ...
        var fileType = RongIMLib.FileType.IMAGE;
          RongIMClient.getInstance().getFileUrl(fileType, data.filename, null, {
            onSuccess: function(result){
              ...
              // sendMessage(msg);
              console.log(vm.user_name,vm.my_user_id,'-------------------------------------------===========-=-=-=+++++++')
              var user={
              name:vm.user_name,
              id:vm.my_user_id,
              portrait:vm.portrait,
              list_name:vm.doctor_name,
              list_portrait:'http://www.hzkygd.cn'+vm.doctor_portrait
            }
             var targetId = vm.$route.query.id
             ...
             RongIMClient.getInstance().sendMessage(conversationtype, targetId, msg, {
                        onSuccess: function (message) {
                            ...
                            my_li.innerHTML= '<img src="'+vm.portrait+'" class="header-img"><div class="message-box"><em><img src="'+imageUri+'"/></em></div>'
            ...
            // }
             ...
             RongIMClient.getInstance().sendMessage(conversationtype, targetId, msg, {
                    onSuccess: function (message) {
                        ...
                        var my_li = document.createElement('li')
                        my_li.className = 'doctor-message'
                        my_li.innerHTML= '<img src="'+vm.portrait+'" class="header-img"><div class="message-box"><em>'+vm.msg+'</em></div>'
    ...
    query_doctor_byPhone:function(){
      ...
      vm.$http.post('/traditionalChineseMedicine/doctor/queryDoctorByPhoneNumber',{
        ...
        vm.doctor_portrait=res.body.data.headImage
